...
# # # # # # # # # # # # # # # # # # # # # 
def cut_mix_label_adaptive0(unlabeled_image, unlabeled_mask, unlabeled_logits, lst_confidences):
    ...
    del mix_unlabeled_image, mix_unlabeled_target, mix_unlabeled_logits
    
    return unlabeled_image, unlabeled_mask, unlabeled_logits 
def cut_mix_label_adaptive1(unlabeled_image, unlabeled_mask, unlabeled_logits, 
    ...
    u_bbx1, u_bby1, u_bbx2, u_bby2 = rand_bbox(unlabeled_image.size(), lam=np.random.beta(4, 4))
    # u_bbx11, u_bby11, u_bbx22, u_bby22 = rand_bbox(unlabeled_image.size(), lam=np.random.beta(4, 4))
    
    # 3) labeled adaptive
    for i in range(0, mix_unlabeled_image.shape[0]):
        #if 0.75 > lst_confidences[i]:
...
def cut_mix_label_adaptive2(unlabeled_image, unlabeled_mask, unlabeled_logits,lst_confidences):
    assert len(lst_confidences) == len(unlabeled_image), "Ensure the confidence is properly obtained"
    #assert labeled_image.shape == unlabeled_image.shape, "Ensure shape match between lb and unlb"
    mix_unlabeled_image1 = unlabeled_image.clone()
    mix_unlabeled_target1 = unlabeled_mask.clone()
    mix_unlabeled_logits1 = unlabeled_logits.clone()
    mix_unlabeled_image2 = unlabeled_image.clone()
    mix_unlabeled_target2 = unlabeled_mask.clone()
    mix_unlabeled_logits2 = unlabeled_logits.clone()
    #labeled_logits = torch.ones_like(labeled_mask)
    ...
    u_bbx11, u_bby11, u_bbx22, u_bby22 = rand_bbox(unlabeled_image.size(), lam=np.random.beta(4, 4))
    for i in range(0, unlabeled_image.shape[0]):
            mix_unlabeled_image1[i, :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]] = \
                unlabeled_image[u_rand_index[i], :, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]]

            mix_unlabeled_target1[i, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]] = \
                unlabeled_mask[u_rand_index[i], u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]]
                
            mix_unlabeled_logits1[i, u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]] = \
                unlabeled_logits[u_rand_index[i], u_bbx1[i]:u_bbx2[i], u_bby1[i]:u_bby2[i]]
            
            
            mix_unlabeled_image2[i, :, u_bbx11[i]:u_bbx22[i], u_bby11[i]:u_bby22[i]] = \
                unlabeled_image[u_rand_index[i], :, u_bbx11[i]:u_bbx22[i], u_bby11[i]:u_bby22[i]]

            mix_unlabeled_target2[i, u_bbx11[i]:u_bbx22[i], u_bby11[i]:u_bby22[i]] = \
                unlabeled_mask[u_rand_index[i], u_bbx11[i]:u_bbx22[i], u_bby11[i]:u_bby22[i]]
                
            mix_unlabeled_logits2[i, u_bbx11[i]:u_bbx22[i], u_bby11[i]:u_bby22[i]] = \
                unlabeled_logits[u_rand_index[i], u_bbx11[i]:u_bbx22[i], u_bby11[i]:u_bby22[i]]
    return mix_unlabeled_image1, mix_unlabeled_target1, mix_unlabeled_logits1, mix_unlabeled_image2, mix_unlabeled_target2, mix_unlabeled_logits2
