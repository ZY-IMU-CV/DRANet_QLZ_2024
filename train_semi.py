...
from dranet.dataset.augs_ALIA import cut_mix_label_adaptive1
from dranet.dataset.augs_ALIA import cut_mix_label_adaptive2
from dranet.dataset.augs_ALIA import cut_mix_label_adaptive0

from dranet.dataset.builder import get_loader
from dranet.models.model_helper import ModelBuilder
from dranet.utils.dist_helper import setup_distributed
from dranet.utils.loss_helper import get_criterion, compute_unsupervised_loss_by_threshold
from dranet.utils.lr_helper import get_optimizer, get_scheduler
from dranet.utils.utils import AverageMeter, intersectionAndUnion, load_state
from dranet.utils.utils import init_log, get_rank, get_world_size, set_random_seed, setup_default_logging
    ...
    for step in range(len(loader_l)):
        ...
        if epoch < cfg["trainer"].get("sup_only_epoch", 0):
            ...
            if np.random.uniform(0, 1) < trigger_prob and cfg["trainer"]["unsupervised"].get("use_cutmix", False):
                assert len(confidence) == len(image_u_aug), "Ensure the confidence is properly obtained"
                for i in range(0, image_u_aug.shape[0]):
                    if cfg["trainer"]["unsupervised"].get("use_cutmix_adaptive", False):
                        if 0.5 > confidence[i]:
                            image_u_aug1, label_u_aug1, logits_u_aug1 = cut_mix_label_adaptive1(
                                    image_u_aug,
                                    label_u_aug,
                                    logits_u_aug, 
                                    image_l,
                            ...
                            if flag_extra_weak:
                                pred_all, aux_all = model(torch.cat((image_l, image_u_weak, image_u_aug1), dim=0))
                                del image_u_aug1
                                pred_l= pred_all[:num_labeled]
                                _, pred_u_strong1 = pred_all[num_labeled:].chunk(2)
                                del pred_all
                            else:
                                pred_all, aux_all = model(torch.cat((image_l, image_u_aug1), dim=0))
                                del image_u_aug1
                                pred_l= pred_all[:num_labeled]
                                pred_u_strong1 = pred_all[num_labeled:]
                                del pred_all
                            unsup_loss, pseduo_high_ratio1 = compute_unsupervised_loss_by_threshold(
                                        pred_u_strong1, label_u_aug1.detach(),
                                        logits_u_aug1.detach(), thresh=p_threshold)
                            pseduo_high_ratio2 = torch.zeros_like(pseduo_high_ratio1)
                            unsup_loss *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)
                            del pred_u_strong1, label_u_aug1, logits_u_aug1
                        elif 0.75 >= confidence[i] >= 0.5:
                            image_u_aug1, label_u_aug1, logits_u_aug1 = cut_mix_label_adaptive0(
                                    image_u_aug,
                                    label_u_aug,
                                    logits_u_aug, 
                                    confidence
                            ...
                            image_u_aug1, label_u_aug1, logits_u_aug1,image_u_aug2, label_u_aug2, logits_u_aug2, = cut_mix_label_adaptive2(
                                    image_u_aug,
                                    label_u_aug,
                                    logits_u_aug, 
                                    confidence
                            ...
                            num_labeled = len(image_l)
                            if flag_extra_weak:
                                pred_all, aux_all = model(torch.cat((image_l, image_u_weak, image_u_aug1, image_u_aug2), dim=0))
                                del image_u_aug1, image_u_aug2
                                pred_l= pred_all[:num_labeled]
                                #_, pred_u_strong = pred_all[num_labeled:].chunk(2)
                                _, pred_u_strong1, pred_u_strong2 = torch.split(pred_all[num_labeled:], [1, 1, 1], dim=0)
                                del pred_all
                            else:
                                pred_all, aux_all = model(torch.cat((image_l, image_u_aug1, image_u_aug2), dim=0))
                                del image_u_aug1, image_u_aug2
                                pred_l= pred_all[:num_labeled]
                                pred_u_strong1, pred_u_strong2 = pred_all[num_labeled:].chunk(2)
                                del pred_all
                            unsup_loss1, pseduo_high_ratio1 = compute_unsupervised_loss_by_threshold(
                                        pred_u_strong1, label_u_aug1.detach(),
                                        logits_u_aug1.detach(), thresh=p_threshold)
                            unsup_loss1 *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)
                            del pred_u_strong1, label_u_aug1, logits_u_aug1
                            unsup_loss2, pseduo_high_ratio2 = compute_unsupervised_loss_by_threshold(
                                        pred_u_strong2, label_u_aug2.detach(),
                                        logits_u_aug2.detach(), thresh=p_threshold)
                            unsup_loss2 *= cfg["trainer"]["unsupervised"].get("loss_weight", 1.0)
                            del pred_u_strong2, label_u_aug2, logits_u_aug2
                            unsup_loss = (unsup_loss1 + unsup_loss2)/2
        ...
        reduced_pseudo_high_ratio1 = pseduo_high_ratio1.clone().detach()
        dist.all_reduce(reduced_pseudo_high_ratio1)
        meter_high_pseudo_ratio.update(reduced_pseudo_high_ratio1.item() / world_size)
        
        reduced_pseudo_high_ratio2 = pseduo_high_ratio2.clone().detach()
        dist.all_reduce(reduced_pseudo_high_ratio2)
        meter_high_pseudo_ratio.update(reduced_pseudo_high_ratio2.item() / world_size)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="config_semi.yaml")
